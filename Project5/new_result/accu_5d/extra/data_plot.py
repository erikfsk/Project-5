...
for txtfile in txtfiles:
	...
	if 0 == int(txtfile[-5:-4]):
		loglog(data["X"],accu_y*100,"k-",label=label_text)
		loglog(data["X"],accu_xy*100,"k--",label=label_text)
		...
		loglog(data["X"],accu_y*100,"m-",label=label_text)
		loglog(data["X"],accu_xy*100,"m--",label=label_text)
		for j in range(4,5,2):
			for i in range(len(accu_xy)):
				if accu_xy[i] > 0.0999*(j+1):
		...
		loglog(data["X"],accu_y*100,"g-",label=label_text)
		loglog(data["X"],accu_xy*100,"g--",label=label_text)
		for j in range(4,5,2):
			for i in range(len(accu_xy)):
				if accu_xy[i] > 0.0999*(j+1):
		...
		loglog(data["X"],accu_y*100,"r-",label=label_text)
		loglog(data["X"],accu_xy*100,"r--",label=label_text)
		for j in range(4,5,2):
			for i in range(len(accu_xy)):
				if accu_xy[i] > 0.0999*(j+1):
